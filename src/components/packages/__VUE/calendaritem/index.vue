<template>
  ...
  >
    ...
    >
      <view class="calendar-title" v-if="showTitle">{{
        title || translate("title")
      }}</view>
      <view class="calendar-top-slot" v-if="showTopBtn">
        <slot name="btn"> </slot>
      </view>
      <view class="calendar-curr-month" v-if="showSubTitle">{{
        yearMonthTitle
      }}</view>
      ...
      <view class="calendar-months-panel" :style="{ heihgt: containerHeight }">
        ...
        >
          <view
            ...
            v-for="(month, index) of compConthsDatas"
            :key="index"
          >
                ...
                <template v-for="(day, i) of month.monthData" :key="i">
                  ...
                  >
                    ...
                    <view
                      ...
                      v-if="isEndTip(day, month)"
                      >{{ endText || translate("end") }}</view
                    >
                  </view>
                </template>
              </view>
  ...
  setup(props, { emit, slots }) {
    ...
    const initData = () => {
      ...
      if (
        ...
        state.currDate =
          props.type != "one" ? [...props.defaultValue] : props.defaultValue;
      }
      // 判断时间范围内存在多少个月
      const startDate = {
      ...
      if (state.defaultData.length > 0) {
        state.monthsData.forEach((item, index) => {
          if (
            item.title ==
            translate("monthTitle", state.defaultData[0], state.defaultData[1])
          ) {
            current = index;
          }
          if (props.type == "range") {
      ...
      if (state.defaultData.length > 0) {
        ...
        if (state.isRange) {
          ...
          [...state.currDate].forEach((item: any) => {
            ...
            state.monthsData.forEach((item, index) => {
              if (
                item.title == translate("monthTitle", dateArr[0], dateArr[1])
              ) {
                current = index;
              }
            });
          ...
          if (props.toDateAnimation) {
            uni
              ...
              .scrollOffset((res) => {
                ...
                let distance =
                  state.monthsData[index].cssScrollHeight - scrollTop;
                let flag = 0;
    ...
    const setDefaultRange = (monthsNum: number, current: number) => {
      ...
      if (JSON.stringify(state.defaultRange) !== JSON.stringify(rangeArr)) {
        ...
        state.compConthsDatas = state.monthsData.slice(
          rangeArr[0],
          rangeArr[1]
        );
      }
      let defaultScrollTop =
        state.monthsData[state.defaultRange[0]].cssScrollHeight;
